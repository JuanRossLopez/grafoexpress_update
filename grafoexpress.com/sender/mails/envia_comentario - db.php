...
include "../connect.php";
function envia_email($nombre, $email, $telefono, $motivo, $ciudad, $comentario, $fecha, $email_empresa){
...
if(!empty($_POST)){
	...
	$busca = mysql_query("SELECT Correo 
	...
	if($inserta_contacto_escolar == true){
		envia_email($nombre_persona, $correo_persona, $telefono_persona, $interes_persona, $ciudad_persona, $comentario_persona, date("Y-m-d H:i:s"), $coreo_devuelto);
		$status[]= array(
			'status' => "1"
			);
	}else{
